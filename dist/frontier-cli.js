...
for (const p of frontier) {
    console.log(`  ${fmt(p.t).padStart(5)}  ${p.value.toFixed(1).padStart(7)}   +${p.name.padEnd(10)} ${p.params.strategy.padEnd(10)} target ${describeComposition(p.target)}`);
}
