...
export function defaultStrategies(data: GameData): Strategy[] {
 ...
 */
export function techClosure(target: Composition, data: GameData, extra: string[] = []): string[] {
...
export function generateBuild(target: Composition, data: GameData, p: BuildParams): Action[] {
  ...
  supUsed += 1;
  probes += 1;
  if (data.economy.hasChrono) A.push(`chrono:${worker}`);
  ...
  if (data.economy.hasChrono && p.secondChrono === "probe") A.push(`chrono:${worker}`);
...
function* enumerateBuilds(target: Composition, data: GameData, map: MapConfig, opts: OptimizeOptions): Generator<Candidate> {
  ...
  const chronoOptions: BuildParams["secondChrono"][] = data.economy.hasChrono ? ["none", "probe", "primary"] : ["none"];
