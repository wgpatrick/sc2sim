...
test("worker count grows past the starting count (regression: probesTotal used to only count 'Probe')", () => {
  for (const [race, data, workerName] of [
    ...
    assert.ok(
      r.final.probes > start,
      `${race}: probesTotal (${r.final.probes}) should exceed starting workers (${start}) after training more`,
    );
  }
});
...
test("MULE income bonus is reflected in mineral bank over its duration", () => {
  ...
  const snapshotAt = (snaps: typeof r1.snapshots, t: number) => {
    for (let i = snaps.length - 1; i >= 0; i--) if (snaps[i].t <= t) return snaps[i].minerals;
...
test("Chrono energy cap scales per Nexus, not a flat 200 total (regression: Scouting Report audit)", () => {
  ...
  assert.ok(
    r.final.energy > PROTOSS.economy.nexusMaxEnergy,
    `2-Nexus energy (${r.final.energy}) should exceed the single-Nexus cap of ${PROTOSS.economy.nexusMaxEnergy}`,
  );
  assert.ok(
    r.final.energy <= PROTOSS.economy.nexusMaxEnergy * 2,
    `2-Nexus energy (${r.final.energy}) should still be capped at 2x a single Nexus`,
  );
});
...
test("valueOverTime only credits a unit with upgrades researched by ITS OWN completion time, not ones the build researches later", () => {
  ...
  assert.ok(
    secondZealotMarginalValue > firstZealotValue,
    `Charge-boosted 2nd Zealot (${secondZealotMarginalValue.toFixed(1)}) should be worth strictly more than the un-upgraded 1st Zealot (${firstZealotValue.toFixed(1)})`,
  );
});
